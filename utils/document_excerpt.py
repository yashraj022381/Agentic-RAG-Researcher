...
def extract_relevant_excerpt(
    ...
    if table_match:
        ...
        for pattern in patterns:
            ...
            if matches:
                # Take the first good match and expand a large window around it
                pos = matches[0].start()
                start = max(0, pos - 800)
                end = min(len(content), pos + 7000)   # large window for tables
