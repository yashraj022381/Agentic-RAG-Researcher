...
class WebSearchTool(BaseTool):
    ...
    def _sanitize_query(query: str) -> str:
        ...
        q = re.sub(r'\\[a-zA-Z]+\{([^}]*)\}', r'\1', q)   # \text{model} -> model
        q = re.sub(r'\\[a-zA-Z]+', '', q)                  # remaining \frac, \sqrt, etc.
        q = re.sub(r'[{}$_^]', ' ', q)                     # leftover braces/$/_/^
    ...
    def run(self, query: str, context = None) -> ToolResult:
        ...
        if not query or not query.strip():
            return ToolResult(
                content="No search query was provided — cannot perform a web search.",
                source="Web: empty-query",
                confidence=0.1,
            )
