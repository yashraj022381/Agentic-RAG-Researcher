...
class KnowledgeBaseTool(BaseTool):
    ...
    def add_documents(self, documents: list, metadatas: list = None):
        ...
        for chunk in KB_CHUNKS:
            overlap = sum(1 for kw in chunk["keywords"] if kw in q)
        ...
        if best_chunk and best_overlap > 0:
            return ToolResult(
                ...
                source = f"KB chunk {best_chunk['id']}",
                confidence = best_chunk["confidence"],
                metadata = {"chunk_id": best_chunk["id"], "keyword_overlap": best_overlap},
            )

        return ToolResult(
            content = (
                f"No highly relevant chunks found for '{query}' in the knowledge base. "
                "Consider searching the web or a broader document corpus."
            ),
            source = "KB: no match",
            confidence = 0.2,
